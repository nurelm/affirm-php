...
use NuRelm\Affirm\Exception\InvalidArgumentException;
use NuRelm\Affirm\Exception\RuntimeException;
...
class AffirmCurl
{
    ...
    public $post_body;
    /**< Post body to send to Affirm */
    public $method;
    /**< HTTP Method to send */
    public $curl;
    /**< cURL handle keeping track of this */
    public $status;
    /**< stores the HTTP status from cURL response */
    public $headers;
    /**< stores the headers from the cURL response */
    public $response;
    /**< stores the raw response body */
    public $response_object;
    /**< response in object form, if available */
    public $options;
    /**< stores the cURL options */
    ...
    public function __construct($url, $method = 'GET', $data = '')
    {
        ...
        if (is_null($url)) {
            throw new \InvalidArgumentException('Missing the URL!');
        }
        $this->status = 0;
        $this->method = $method;
        $this->url = $url;
    ...
    public function send()
    {
        ...
        if ($success) {
            ...
            throw new RuntimeException('Unable to send cURL request');
        }
    }
